 ...
 */
function slash_form_install_configure_form_alter(&$form, $form_state) {
  // Pre-populate the site name with the server name.
  $form['site_information']['site_name']['#default_value'] = $_SERVER['SERVER_NAME'];
  $form['#submit'][] = 'slash_form_submit';
}


function slash_form_submit($form, &$form_state) {
  // Import database slash file.
  $slash_file = dirname(__FILE__) . '/slash.sql';
  $success = import_slash($slash_file);
...
/**
 * Imports a database slash file.
 *
 * @see demo_reset().
 */
function import_slash($filename) {
  ...
  if (!file_exists($filename) || !($fp = fopen($filename, 'r'))) {
    drupal_set_message(t('Unable to open slash file %filename.', array('%filename' => $filename)), 'error');
    return FALSE;
  }
