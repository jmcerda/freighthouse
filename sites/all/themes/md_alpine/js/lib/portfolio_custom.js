...
// LOAD PROJECT
    ...
    function initializePortfolio() {
        ...
        $(window).bind('hashchange', function() {
            ...
            if (hash.substr(0, rootLength) != root) {
                ...
                if (pageRefresh == true && hash.substr(0, rootLength) == root) {
                    ...
                    var pjContainer = $("div#ajax-content-inner");
                    $('html,body').stop().animate({
                        scrollTop : (pjContainer.offset().top - headerH) + 'px'
                    }, 800, 'easeOutExpo', function() {
        ...
        function loadProject() {
            $('div#loader').fadeIn().removeClass('projectError').html('');
            if (!ajaxLoading) {
                ajaxLoading = true;
                var pjContainer = $("div#ajax-content-inner");
                pjContainer.load(url + ' div#ajaxpage', function(xhr, statusText, request) {
                    if (statusText == "success") {
                        ...
                        jQuery('div#ajaxpage').waitForImages(function() {
                            hideLoader();
                        });
        ...
        function showProject() {
            if (content == false) {
                var pjContainer = $("div#ajax-content-inner");
                wrapperHeight = pjContainer.children('div#ajaxpage').outerHeight() + 'px';
                pjContainer.animate({
                    opacity : 1,
                    height : wrapperHeight
                }, function() {
        ...
        /* CLOSE PROJECT */
        $('#closeProject a').on('click', function(event) {
            event.preventDefault();
            deleteProject($(this).attr('href'));
            portfolioGrid.find('div.portfolio-item.current').children().removeClass('active');
            loader.fadeOut();
            //$('html, body').delay(1000).animate({ scrollTop: $("#filters").offset().top - 100}, 800);
        });
        pageRefresh = false;
